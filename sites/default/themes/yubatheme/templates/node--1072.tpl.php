...
<?php 
	...
	$q = 'SELECT field_data_field_project_coordinates.entity_id AS nid, node.title AS title, field_project_coordinates_lat AS lat, field_project_coordinates_lon AS lon FROM field_data_field_project_coordinates, node WHERE node.nid = field_data_field_project_coordinates.entity_id;';
	...
	foreach($results as $result) {
	   ...
	   echo "{nid: {$result->nid}, title: '" .htmlentities($result->title, ENT_QUOTES) ."', lat: {$result->lat}, lon: {$result->lon}, stationcat: 'Yuba River Watershed', url: '" . 
...
//		        return siteColors[colorSelector];
		    ...
		    .on("mouseover", function(d) { 
			    ...
			    var content = '<div class="popup-output"><a href="' + d.url + '">' + d.title + '</a></div>';//<br />' + photo + '</div>';
			    infowindow.setContent(content);
			    infowindow.setPosition(new google.maps.LatLng(d.lat, d.lon));
			    infowindow.open(map);
				jQuery('.popup-output').parent().css({'overflow':'hidden', 'width': '254px'});
...
var subbasins = new google.maps.KmlLayer({'url': 'http://dev.yubashed.org/sites/default/files/subwatersheds-symbolized.kmz', 'preserveViewport': 'true'});
